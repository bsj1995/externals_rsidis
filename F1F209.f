...
C Modified by Dave G. to use Eric Christy's F1F220 fits to the proton and neutron structure functions
!--------------------------------------------------------------------
      implicit none
      REAL*8 P(0:23)
      COMMON/PARCORR/P
      real*8 Z,A,qsq,wsq,w,f1c,x
      real*8 avgn,r,dr,nu,eps,kappa,sigres,flux,siglp,sigtp,F1pp,F1dp,F2pp,F2dp
      real*8 W1p,W1pp,W2p,W2pp,W1n,W1np,W2n,W2np,F1n,F2n,F1np,F2np
      real*8 W1tmp,W2tmp,F1tmp,F2tmp
      REAL*8 W1,W2,sigt,rc,w1pb,w2pb,F1,F2,sigl,F1d, F2d,F1p,F2p,qv
      REAL*8 DW2DPF,WSQP,pf,kf,es,dw2des,fyuse
      real a4, x4, fitemc_rock
      real*8 emcfac,emcfit_e139
      logical goodfit
      INTEGER ISM
...
! For deuteron
      if(IA .eq. 2) then
         call F1F2IN21(Z, A, QSQ, WSQ, F1d, F2d)
ccc convert to W1 per deuteron
         W1 = F1d / PM
         W2 = F2d/nu
      endif
...
cc          WSQP = WSQ + XX(ISM) * PF * DW2DPF - es * dw2des
        do ism = 1,99
          ...
          IF(WSQP.GT. 1.159) THEN
             call F1F2IN21(1.0d0, 1.0d0, QSQ, WSQp, F1pp, F2pp)
             call F1F2IN21(0.0d0, 1.0d0, QSQ, WSQp, F1np, F2np)
             F1n = F1n + F1np *Fyuse
             F1p = F1p + F1pp *Fyuse
c             W1p = F1pp/PM 
c             W2p = F2pp/nu
c             sigt=sigt + 
c     >               2.0*PM*(4.0*PI**2*alpha)/(WSQ-PM**2)*W1p *Fyuse
c             sigl = sigl+ sigt*((W2p/W1p)*(1.0+nu**2/QSQ)-1.0) *Fyuse
          ENDIF
        ENDDO
        Rc = 0.
c just assume RA=RD for now        
        call F1F2IN21(1.0d0, 2.0d0, QSQ, WSQ, F1tmp, F2tmp)
        w1tmp=F1tmp/PM
        w2tmp=F2tmp/nu
        Rc=(W2tmp/W1tmp)*(1.0+nu**2/QSQ)-1.0
        W1=((A-Z)*(F1n) + Z*F1p)/PM
...
c        if(Wsq .gt.0.0 ) Rc = Rc * ( 1.0 + P(6) + P(23)*A )
     ...
     >        (1.0+P(13)*x+P(14)*x**2+P(15)*x**3+P(16)*x**4+P(17)*x**5)
      ...
      x4 = qsq / 2. / pm / nu
      x = qsq / 2. / pm / nu
c      emcfac = fitemc_rock(x4, a4, goodfit)
      emcfac = emcfit_e139(x, a)
...
! Modified value by DG to improve agreement with XEM data
      if(a.gt.2.5 .and. a.lt.3.5) p18 = 47.0
...
c new 5/07 values. Values 1 and 7 will be overridden below.
     ...
     & 0.10936E+00,0.37944E+00/
      ...
      sig = 0.
c      if(w2.lt.1.07327**2 .or. w2.gt.25 .or. 
c     >  q2.lt.0.0 .or. q2.gt.11.0) then
c        write(6,'(1x,''error, q2 or w2 out of range'',2f8.3)') w2,q2
c        return
c      endif
...
!Term    Coeficient     Error                                          
     ...
     >  5.04268396D-03,    1.406D-03   /                                
      ...
      IF( (X.GT.0.80).OR.(X.LT. 0.0085) ) THEN   !Out of range of fit   
       IF(X.LT. 0.0085) X_U =.0085
       IF(X.GT. 0.80) X_U = 0.80
       GOODFIT=.FALSE.
      ELSE
       X_U=X
       GOODFIT=.TRUE.
...
! changed to use max(0.3,q2)
     ...
     >   (1. + max(0.3,q2) / xval(4))**xval(5) * nu ** xval(6)
      ...
      real*8 function emcfit_e139(xpass,A)
      implicit none
      
      real*8 alpha,C,x,xpass,A

      if (xpass.gt.1.0) then
         x=1.0
      else
         x=xpass
      endif
