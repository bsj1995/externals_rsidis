...
c     >        E0SET,EPSET,TH,SIGMA_BORN,sigma_corr
         ...
         if(SIGMA_BORNPLSQE.le.0.0) then
            dorad=.false.
         else
            dorad=.true.
         endif
...
c also check for NaN
           if(isnan(sbsv(npts))) sbsv(npts)=0.0
           if(isnan(sbisv(npts))) sbisv(npts)=0.0
           if(isnan(sbqesv(npts))) sbqesv(npts)=0.0
           if(isnan(srsv(npts))) srsv(npts)=0.0            
           if(isnan(srelsv(npts))) srelsv(npts)=0.0            
           if(isnan(srqesv(npts))) srqesv(npts)=0.0            
           if(isnan(srinsv(npts))) srinsv(npts)=0.0            
...
! z in cm relative to center 
 ...
 125    format(1x,'CRYO22 rad len=',5f10.5)
      ...
      IF(INDEX(TARGET,'LAD').GT.0) THEN     ! target geometry for
                                               ! 20 cm LD2
        ECIR=0.3935*2.54                        ! endcap inner radius (cm
        ECOR=0.4077*2.54                        ! endcap outer radius (cm
        TARGLEN=20.00                          ! target length (cm
        ENTEC=TARGLEN-ECIR                     ! entrance to end cap (cm
        TBAL=TBEAM                             ! material bf the target
        TBEFOR=T                               ! distance traveled bf
                                               ! vertex
     ...
     >    *TWALL/(ECOR-ECIR)                   ! & end cap
        ...
        nprnt= nprnt+1
        if(nprnt.le.6) write(*,126) tcm,tbal,tbefor,tafter,taal
 126    format(1x,'LAD rad len=',5f10.5)
      ENDIF
